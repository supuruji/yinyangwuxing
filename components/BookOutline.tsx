...
function groupCards(cards: BookOutlineCard[]): BookOutlineCard[][] {
  const groups: Record<string, BookOutlineCard[]> = {};
  const order: string[] = [];
...
export default function BookOutline({
  ...
  return (
    <div className="space-y-10">
      ...
      {groups.map((group) => (
        <section key={group[0].group} className="space-y-2">
          ...
          <div className="grid gap-2">
            {group.map((item) => (
              <div key={item.chapter} className="rounded-md border border-gold/30 bg-ink-card px-3 py-3 sm:px-4 sm:py-4 hover:border-gold/70 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4">
                  <div className="min-w-0 flex-1 flex items-start gap-3">
                    <span className="shrink-0 mt-0.5 inline-flex items-center justify-center w-7 h-7 rounded-full border border-gold/40 text-gold text-xs font-serif">
                      {item.code}
                    </span>
                    <h3 className="text-parchment font-serif leading-snug text-base sm:text-lg">
                      {item.title}
                    </h3>
                  </div>
                  <div className="flex flex-wrap gap-1.5 shrink-0">
                    ...
                    <Link
                      href={`/${locale}/books/${bookId}?read=${item.chapter}`}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 border border-gold/50 hover:border-gold hover:bg-gold/10 text-parchment text-xs rounded transition-colors"
