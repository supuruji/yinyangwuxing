...
import { getZhIndex, getZhSegment, ZH_TOTAL } from '@/lib/dissertationZh';
...
export function generateStaticParams() {
  return Array.from({ length: ZH_TOTAL }, (_, i) => ({ seg: String(i) }));
}
...
function isHeading(line: string): boolean {
  ...
  if (
    ...
    return true;
  // short title-like lines (no comma) are treated as headings
...
export default async function SegmentPage({ params }: Props) {
  ...
  if (Number.isNaN(id) || id < 0 || id >= ZH_TOTAL) notFound();

  const segment = getZhSegment(id);
  const index = getZhIndex();
  const base = `/${locale}/dissertation/doctoral/donghak-daesoon`;
  const prevId = id > 0 ? id - 1 : null;
  const nextId = id < ZH_TOTAL - 1 ? id + 1 : null;
