...
const UI = {
  ko: { dissertations: '학위논문', masters: '석사논문', footnotes: (n: number) => `각주 ${n}개` },
  en: { dissertations: 'Dissertations', masters: "Master's", footnotes: (n: number) => `${n} footnotes` },
  zh: { dissertations: '学位论文', masters: '硕士论文', footnotes: (n: number) => `脚注 ${n}` },
  ja: { dissertations: '学位論文', masters: '修士論文', footnotes: (n: number) => `脚注 ${n}件` },
} as const;
...
export default async function MastersOverviewPage({ params }: Props) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <nav className="text-xs text-parchment-muted mb-6 flex items-center gap-1.5">
        ...
        <Link href={`/${locale}/dissertation/masters`} className="hover:text-gold transition-colors">{ui.masters}</Link>
        <span className="text-gold/40">›</span>
      ...
      <div className="space-y-3">
        {meta.chapters.map((ch: { id: string; title: string; paragraphCount: number; footnoteCount: number }) => (
          <Link
            key={ch.id}
            href={`/${locale}/dissertation/masters/${dissertationId}/${ch.id}`}
